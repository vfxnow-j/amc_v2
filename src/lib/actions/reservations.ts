...
export type ReservationFormData = {
  ...
  // Date the quoted pricing stops being honoured (null clears it)
...
async function maybeAutoInvoice(tx: TxClient, reservationId: string): Promise<boolean> {
  ...
  // Check existing invoices (excluding voided/cancelled) to avoid double-billing
  ...
  // the old asset-level behaviour back when an asset could only appear once.
...
export async function updateReservationItemRate(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(reservation.status)) {
      throw new Error('Cannot adjust rates on completed, cancelled, or lost reservations')
    }
...
export async function updateReservationItemQuantity(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (!reservation) throw new Error('Reservation not found')
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(reservation.status)) {
      throw new Error('Cannot adjust quantity on completed, cancelled, or lost reservations')
    }
...
export async function addItemToReservation(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!reservation) throw new Error('Reservation not found')
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(reservation.status)) {
      throw new Error('Cannot add items to completed, cancelled, or lost reservations')
    }
...
export async function updateCloudHostInReservation(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!reservation) throw new Error('Reservation not found')
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(reservation.status)) {
      throw new Error('Cannot edit items on completed, cancelled, or lost reservations')
    }
...
export async function reorderReservationItems(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (!reservation) throw new Error('Reservation not found')
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(reservation.status)) {
      throw new Error('Cannot reorder items on completed, cancelled, or lost reservations')
    }
...
export async function cancelReservation(id: string, reason?: string) {
  ...
  const reservation = await prisma.$transaction(async (tx) => {
    ...
    if (['COMPLETED', 'CANCELLED', 'LOST'].includes(existingReservation.status)) {
      throw new Error('Cannot cancel completed, already cancelled, or lost reservations')
    }
  ...
  // Sync cancelled status to HubSpot (fire-and-forget)
  syncReservationDeal({
    id: reservation.id,
...
export async function deleteReservation(id: string) {
  ...
  if (existing.status !== 'CANCELLED') throw new Error('Only cancelled reservations can be deleted')
